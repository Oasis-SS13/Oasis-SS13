...
/datum/guardian_ability/major/special/explosive
	name = "Explosive requiem"
	desc = "The guardian can, with a single touch, turn any inanimate object into a bomb."
	ui_icon = "bomb"
	cost = 4
	spell_type = /obj/effect/proc_holder/spell/aimed/straycat
	var/bomb_cooldown = 0
	var/static/list/boom_signals = list(COMSIG_PARENT_ATTACKBY, COMSIG_ATOM_BUMPED, COMSIG_ATOM_ATTACK_HAND)

/datum/guardian_ability/major/special/explosive/Apply()
	. = ..()
	guardian.verbs += /mob/living/simple_animal/hostile/guardian/proc/DetonateBomb

/datum/guardian_ability/major/special/explosive/Remove()
	. = ..()
	guardian.verbs -= /mob/living/simple_animal/hostile/guardian/proc/DetonateBomb

/datum/guardian_ability/major/special/explosive/Attack(atom/target)
...
/datum/guardian_ability/major/special/explosive/AltClickOn(atom/A)
	if(!istype(A))
		return
...
/datum/guardian_ability/major/special/explosive/proc/kaboom(atom/source, mob/living/explodee)
	if(!istype(explodee))
		return
...
/datum/guardian_ability/major/special/explosive/proc/disable(atom/A)
	to_chat(src, "<span class='danger'><B>Failure! Your trap didn't catch anyone this time.</B></span>")
	guardian.bombs -= A
	UNREGISTER_BOMB_SIGNALS(A)

/datum/guardian_ability/major/special/explosive/proc/display_examine(datum/source, mob/user, text)
	text += "<span class='holoparasite'>It glows with a strange <font color=\"[guardian.guardiancolor]\">light</font>!</span>"


/mob/living/simple_animal/hostile/guardian/proc/DetonateBombs()
