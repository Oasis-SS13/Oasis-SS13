...
/mob/dead/new_player/proc/LateChoices()
	...
	for(var/list/category in list(GLOB.command_positions) + list(GLOB.engineering_positions) + list(GLOB.supply_positions) + list(GLOB.nonhuman_positions - "pAI") + list(GLOB.civilian_positions) + list(GLOB.gimmick_positions) + list(GLOB.medical_positions) + list(GLOB.science_positions) + list(GLOB.security_positions))		var/cat_color = "#ffffff" // white by default.
		var/exp_type = EXP_TYPE_CREW
    var/cat_color = "#ffffff" // white by default.
		if(SSjob.name_occupations[category[1]])
		  cat_color = SSjob.name_occupations[category[1]].selection_color //use the color of the first job in the category (the department head) as the category color
			exp_type = SSjob.name_occupations[category[1]].exp_type_department
		dat += "<fieldset style='width: 185px; border: 2px solid [cat_color]; display: inline'>"
		dat += "<legend align='center' style='color: [cat_color]'>[exp_type]</legend>"
